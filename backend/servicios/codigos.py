...
def siguiente_codigo(db: Session, model, campo: str, prefijo: str) -> str:
    """Siguiente codigo tipo PREFIJO-0001, buscando el maximo ya usado.
    ...
    maximo = 0
    for (valor,) in db.query(columna).filter(columna.like(f"{prefijo}-%")).all():
        if not valor:
            continue
