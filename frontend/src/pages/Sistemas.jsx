...
const Sistemas = () => {
  ...
  const fetchSistemas = () => {
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/sistemas/')
      .then(res => res.json())
  ...
  const handleAddSistema = (e) => {
    ...
    const url = isEditing ? `http://localhost:8000/sistemas/${newSistema.id}` : `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/sistemas/';
  ...
  const handleAddClasificacion = (e) => {
    ...
    const url = isEditing ? `http://localhost:8000/clasificaciones/${newClasificacion.id}` : `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/clasificaciones/';
    const method = isEditing ? 'PUT' : 'POST';
