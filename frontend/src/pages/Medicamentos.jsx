...
const Medicamentos = () => {
  ...
  const fetchMedicamentos = () => {
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/medicamentos/')
      .then(res => res.json())
  ...
  const handleAddMed = (e) => {
    ...
    const url = isEditing ? `http://localhost:8000/medicamentos/${newMed.id}` : `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/medicamentos/';
  ...
  const handleDeleteMed = (id) => {
    if (window.confirm('¿Eliminar medicamento del catálogo?')) {
      fetch(`http://localhost:8000/medicamentos/${id}`, { method: 'DELETE' })
  ...
  const openModal = (type, data = null) => {
    ...
    if (type === 'med') {
      ...
      fetch(`http://localhost:8000/kardex/${data.id}`)
        .then(res => res.json())
