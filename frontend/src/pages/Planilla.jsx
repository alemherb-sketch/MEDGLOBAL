...
const Planilla = () => {
  ...
  const fetchTrabajadores = () => {
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/trabajadores/')
      .then(res => res.json())
  ...
  const handleSubmit = (e) => {
    ...
    const url = isEditing ? `http://localhost:8000/trabajadores/${formData.id}` : `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/trabajadores/';
