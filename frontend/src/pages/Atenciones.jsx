...
const Atenciones = () => {
  ...
  const fetchData = () => {
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/atenciones/').then(res => res.json()).then(setAtenciones);
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/trabajadores/').then(res => res.json()).then(setTrabajadores);
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/sistemas/').then(res => res.json()).then(setSistemas);
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/citas/').then(res => res.json()).then(setCitas);
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/personal_salud/').then(res => res.json()).then(setPersonalSalud);
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/medicamentos/').then(res => res.json()).then(setMedicamentos);
  };
  ...
  const handleAddAtencion = (e) => {
    ...
    const url = isEditing ? `http://localhost:8000/atenciones/${newAtencion.id}` : `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/atenciones/';
