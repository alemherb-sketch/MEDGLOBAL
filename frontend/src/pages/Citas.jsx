...
const Citas = () => {
  ...
  const fetchData = () => {
    ...
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/trabajadores/').then(res => res.json()).then(setPacientes);
  ...
  const handleSubmit = (e) => {
    ...
    const url = isEditing ? `http://localhost:8000/citas/${formData.id}` : `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}`/citas/';
  ...
  const handleDelete = () => {
    if (window.confirm('¿Eliminar esta cita médica?')) {
      fetch(`http://localhost:8000/citas/${formData.id}`, { method: 'DELETE' })
        .then(() => {
          fetchData();
          setIsModalOpen(false);
        });
